...
our %config = (
    ...
    "openssl_sys_defines" => [],
    "openssldir" => "",
    "options" => " no-acvp-tests no-afalgeng no-apps no-asan no-asm no-async no-brotli no-brotli-dynamic no-buildtest-c++ no-capieng no-comp no-crypto-mdebug no-crypto-mdebug-backtrace no-ct no-demos no-devcryptoeng no-dso no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-engine no-err no-external-tests no-fips no-fips-jitter no-fips-post no-fips-securitychecks no-fuzz-afl no-fuzz-libfuzzer no-h3demo no-jitter no-ktls no-loadereng no-md2 no-module no-msan no-ocsp no-padlockeng no-pie no-psk no-rc5 no-rfc3779 no-sctp no-shared no-srp no-srtp no-ssl-trace no-ssl3 no-ssl3-method no-sslkeylog no-stdio no-tests no-tfo no-trace no-ts no-ubsan no-unit-test no-uplink no-weak-ssl-ciphers no-winstore no-zlib no-zlib-dynamic no-zstd no-zstd-dynamic",
    "patch" => "0",
...
unless (caller) {
    ...
    use lib '/openssl/util/perl';
    use OpenSSL::fallback '/openssl/external/perl/MODULES.txt';
    ...
    if (scalar @ARGV == 0) {
        ...
        use lib '.';
        use lib '/openssl/Configurations';
        use gentemplate;
